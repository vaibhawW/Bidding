...
<div class="container p-5">
    ...
    <form action="{{isset($manager)?route('manager.update',['id'=>$manager->id]):route('manager.store')}}" method='{{isset($manager)?"PUT":"POST"}}'>
        {{csrf_field()}}
        @if(isset($manager))
        ...
        <div class="input-group mb-3">
            <div class="input-group-prepend">
                <span class="input-group-text" id="basic-addon3">Manager Name</span>
            </div>
            <input type="text" class="form-control" name="name" id="basic-url" aria-describedby="basic-addon3" value="{{$manager->name or ""}}" autofocus>
        </div>
        <label for="details">Description Of Manager</label>
        <textarea placeholder="Description of Manager" name="details">{{$manager->details or ""}}</textarea>
        <input type="submit" class="btn btn-success mt-1 right" value="{{isset($manager)?'Update Manager':'Add Manager'}}">
    </form>
</div>
...
<script>
    ...
    $.toast({
        heading: 'Success',
        text: "Manager's details Updated",
        showHideTransition: 'slide',
        position: 'top-right',
        icon: 'success'
    });
